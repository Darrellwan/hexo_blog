...
pubDatetime: 2026-04-09T10:43:34+08:00
modDatetime: 2026-05-04T10:00:00+08:00
description: Claude Managed Agents 完整實測！Anthropic 雲端 AI Agent 託管服務，不用自己架基礎架構。實測 Quickstart 流程、Template 範本、定價拆解、多 Agent 協作 Research Preview，搭配 MCP Server 整合分享。
tags:
  - Claude
  - AI Agent
  - Anthropic
...
{% darrellImage800Alt "Claude Managed Agents 在 Anthropic Console 左側選單的位置（標示 New）" claude_console_navigation_menu.png max-400 %}
...
{% darrellImage800Alt "Claude Managed Agents Browse Templates 範本選擇頁面，顯示多種預建 AI Agent 範本" console_browse_templates.png max-800 %}

我先選擇用 Deep Researcher 這個 Template

{% darrellImage800Alt "Claude Managed Agents 的 Agent Config YAML 設定畫面" multi_step_web_research_agent_configuration.png max-800 %}
...
{% darrellImage800Alt "Claude Managed Agents Transcript 分頁顯示完整對話紀錄和工具呼叫" claude_managed_agents_deep_research_session.png max-800 %}
...
{% darrellImage800Alt "Claude Managed Agents Debug 分頁顯示完整事件時間軸" claude_managed_agents_interaction_preview.png max-800 %}
...
[
  ...
  {"工具": "MCP Servers", "用途": "連接外部服務（可參考我寫過的 <a href='/line-mcp-server/'>LINE MCP Server</a>）"},
...
跟 [Claude Code](/claude-code-channels-discord-telegram/) 的 memory 功能概念一樣，只是變成 API 版本
...
{% darrellImage800Alt "Claude Managed Agents 定價拆解：Console Cost 頁面顯示 Total token cost $2.19，Session runtime $0.01" console_cost_dashboard.png max-400 %}
...
如果你對 Claude 生態系跟 AI Agent 有興趣，這幾篇可以接著看：

{% articleCard url="/claude-cowork-intro/" title="Claude Cowork 教學：給大眾用的 Claude Code" previewText="不寫程式也能用 Claude 做事的版本，跟 Managed Agents 走在不同光譜的兩端。" %}

{% articleCard url="/claude-code-channels-discord-telegram/" title="Claude Code Channels 用 Discord、Telegram 遠端使用 Claude Code" previewText="把 Claude Code 接到 IM 的另一種 Agent 體驗。" %}

{% articleCard url="/line-mcp-server/" title="LINE MCP Server 測試心得" previewText="MCP Server 怎麼跟 Claude 接起來，Managed Agents 也能掛 MCP。" %}
...
[
  ...
  {"question": "跟 Claude Code 有什麼不同？", "answer": "<a href='/claude_code_update_202509/'>Claude Code</a> 是本機端的開發工具，Managed Agents 是雲端 API 服務。如果你是一般使用者（不是工程師），可以看看 <a href='/claude-cowork-intro/'>Claude Cowork</a> 這個給大眾用的版本。"},
