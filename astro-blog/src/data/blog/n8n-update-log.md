...
modDatetime: 2026-08-26T22:08:28+08:00
description: n8n 2.37.0（Pre-release）更新紀錄，整理 Anthropic Prompt Caching 選項、Message an Agent 串流開關與限制，以及 NDV 節點詳細視窗新增的停止目前執行按鈕，目前正式版本為 2.36.7
tags:
  - n8n
  - update_log
categories:
...
## 2.37.0 Pre-release - 2026-08-25

[Github 2.37.0 更新](https://github.com/n8n-io/n8n/releases/tag/n8n%402.37.0)

### Anthropic Prompt Caching 讓重複 Prompt 少花一點
**Anthropic Chat Model Node:** Add prompt caching support

如果 workflow 每次都把同一段 system prompt、工具定義或很長的背景資料送給 Anthropic
這些固定內容每次請求都會重複計算

2.37.0 在 Anthropic Chat Model 的 Options 裡新增 **Prompt Caching**
目前有三個選項可以選：**Disabled**、**5 Minutes** 和 **1 Hour**

在預覽站把選項設成 5 Minutes
連續跑兩次相同測試都能順利執行完成
不過目前 n8n 畫面只會顯示合計的 tokenUsage，沒有分出 cache creation 和 cache read
所以畫面上沒辦法直接確認快取命中率或省了多少 token

照官方機制，第一次把內容寫進 cache 成本會比較高，後續在時效內重複命中才划算
固定 prompt、工具定義和長背景資料比較適合開這個
（另外 n8n 介面顯示的 token 數是這次請求的合計用量，不等於實際帳單）

{% darrellImage800Alt "n8n 2.37.0 Anthropic Chat Model 節點開啟 Prompt Caching，Prompt Caching 選擇 5 Minutes" n8n-2.37.0-anthropic_prompt_caching.png max-800 %}

### Message an Agent 可以開啟串流回覆
Add streaming to Message an Agent workflows

Message an Agent 如果要等整個 Agent workflow 跑完才一次回傳
遇到需要處理比較久的流程，使用者只能在前面乾等

這版在 **Message an Agent → Advanced** 裡新增 **Enable Streaming**
開啟存檔後設定會保留

在 editor 跑測試時可以看到 `Executing node...` 的執行進度
不過這項功能目前屬於 **Agents Preview**
自架 self-hosted 需要先啟用 agents module，而且不支援 queue mode
如果同時開了 Structured Output，文字 streaming 就會自動停用

{% darrellImage800Alt "n8n 2.37.0 Message an Agent 節點 Advanced 設定中的 Enable Streaming 開關" n8n-2.37.0-message_agent_streaming.png max-800 %}

### 節點詳細視窗新增停止目前執行按鈕
**editor:** Add stop execution button to the NDV

在 NDV 裡手動測試 Wait 或很花時間的節點時
如果不小心設錯或跑太久，往往不一定想等它整段跑完

2.37.0 直接把停止控制放進節點視窗
執行時會看到 **Stop current execution**

實測跑一個 Wait 120 秒
按下停止後執行狀態會變成 `Canceled`
畫面上也會顯示 `The execution was cancelled manually`

這樣在編輯器裡測試卡住或不想繼續等的節點時
就能直接中止，不用離開目前的節點視窗

{% darrellImage800Alt "n8n 2.37.0 NDV 執行 Wait 節點時顯示 Stop current execution 停止按鈕" n8n-2.37.0-ndv_stop_execution.png max-800 %}
