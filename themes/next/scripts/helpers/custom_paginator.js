...
function customPaginatorHelper(options = {}) {
  ...
  if (prevNext && current > 1) {
    tags.push(htmlTag('a', { class: prevClass, 'aria-label':prevAria , rel: 'prev', href: link(current - 1)}, prevText, escape));
  ...
  if (prevNext && current < total) {
    tags.push(htmlTag('a', { class: nextClass, 'aria-label':nextAria , rel: 'next', href: link(current + 1) }, nextText, escape));
  } else if (forcePrevNext) {
...
hexo.extend.helper.register("custom_paginator", customPaginatorHelper);
