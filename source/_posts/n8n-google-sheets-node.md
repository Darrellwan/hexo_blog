---
title: n8n Google Sheets 節點教學 - 自動化試算表讀寫與同步
tags:
  - n8n
  - n8n節點介紹
  - n8n教學
...
description: 完整教學 n8n Google Sheets 節點。實測 Append、Append or Update、Get Rows 三大操作、OAuth2 vs Service Account 選擇、Trigger polling 限制、AI Agent 掛 Sheets 當 Tool 新特性，搭配 Gmail 電子發票自動記錄實戰案例。
bgImage: blog-n8n-google-sheets-bg-v2.png
preload:
  - blog-n8n-google-sheets-bg-v2.png
date: 2026-04-21 10:00:00
modified: 2026-04-21 10:00:00
---

{% darrellImageCover n8n-google-sheets-node-bg blog-n8n-google-sheets-bg-v2.png max-800 %}
...
{% darrellImage800Alt "Google Sheets 節點 Operation 決策樹" sheets_operation_decision_tree.png max-800 %}
...
⚠️ 不要選「金額」當 match column：不同店家的發票金額可能一樣（例如剛好都是 79 元），選金額當比對欄位的話，新的 79 元發票進來就會把舊的那筆蓋掉。
...
<h2 id="ai-tool">延伸閱讀：Sheets 當 AI Agent Tool</h2>

n8n 1.62.1（2024 年 10 月）之後，Google Sheets 節點支援 `usableAsTool: true`，可以直接掛到 AI Agent 節點當 Tool 使用。

{% darrellImage800Alt "Google Sheets 節點掛進 AI Agent 當 Tool 示意" n8n_workflow_google_sheets_ai_agent_automation.png max-800 %}

掛上去之後，AI Agent 可以自己決定「現在要不要查試算表」「要不要寫一筆資料」，不需要你在 workflow 裡預先寫死每一步的邏輯。適合的場景：讓 AI 根據對話內容判斷要更新哪一列、或是讓 AI 自己去 Sheet 查資料再回答問題。

這個玩法的細節（Tool Schema 設計、Prompt 怎麼寫、Error handling）會另外寫專文。這裡先讓你知道 Google Sheets 節點有這個能力，到時候有興趣可以接著看。
